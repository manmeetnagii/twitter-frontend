...
const MainProfile = ({ user }) => {
  ...
  useEffect(() => {
    fetch(`https://twitter-backend-7uv3.onrender.com/userpost?email=${user?.email}`)
      .then(res => res.json())
      .then(data => {
        setPosts(data);
      })
  ...
  const handleUploadCoverImage = e => {
    ...
    axios.post("https://api.imgbb.com/1/upload?key=da3582ad3c3d7547a13463af5c430fae", formData)
      .then(res => {
        ...
        if (url) {
          fetch(`https://twitter-backend-7uv3.onrender.com/userUpdates/${user?.email}`, {
            method: "PATCH",
            headers: {
        ...
        <div className='profile-bio'>
          {
            <div >
              ...
              {
                posts.map(p => <Post p={p} key={p.username}/>)
              }
            </div>
          }
        </div>
